...
本文描述节点 1 已确认的 MealOps V1 技术架构、模块边界、依赖方向和运行关系。它是后续工程初始化与纵向业务节点的约束，不代表仓库中已经存在后端、前端、数据库或部署实现。

本文不设计具体数据库表、API 字段或 Planner 算法参数。尚未确认的细节继续保留到对应节点通过 ADR 决策。
...
负责标准食材、别名、单位维度和允许的换算关系，为菜谱、库存和购物提供统一食材身份与数量语义。
