...
@Mapper public interface MealPlanMapper {
 ...
 @Insert("INSERT INTO meal_plan_slot(meal_plan_id,meal_date,meal_type,recipe_id,target_servings) VALUES(#{planId},#{mealDate},#{mealType},#{recipeId},#{targetServings})") int insertSlot(@Param("planId") long planId,@Param("mealDate") LocalDate date,@Param("mealType") String type,@Param("recipeId") Long recipeId,@Param("targetServings") Integer servings);
 ...
 @Select("SELECT meal_date AS mealDate,meal_type AS mealType,recipe_id AS recipeId,target_servings AS targetServings FROM meal_plan_slot WHERE meal_plan_id=#{id} ORDER BY meal_date, CASE meal_type WHEN 'BREAKFAST' THEN 0 WHEN 'LUNCH' THEN 1 WHEN 'DINNER' THEN 2 END") List<MealSlotRow> findSlots(long id);
