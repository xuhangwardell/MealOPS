...
public class MyBatisInventoryBatchRepository implements InventoryBatchRepository {
    ...
    private InventoryBatch toDomain(InventoryBatchEntity e) { Unit u = Unit.fromCode(e.getUnitCode()).orElseThrow(() -> new IllegalStateException("Unknown persisted unit: " + e.getUnitCode())); return InventoryBatch.reconstitute(e.getId(), e.getIngredientId(), Quantity.of(e.getRemainingAmount(), u), e.getExpiresOn()); }
}
